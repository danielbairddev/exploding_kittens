"""Rollout workers for Gabriel self-play.

Gabriel is trained to die FIRST — the reward is inverted relative to Elephant.
A +1 reward is given when Gabriel is the first player to explode; 0 otherwise.
"""
import numpy as np
...
def play_one(policy_w, pool_w, rng, npr, self_prob=0.1):
    p = _np(policy_w)
    ...
    # Gabriel wins by dying FIRST — +1 if first to explode, 0 otherwise
    reward = 1.0 if deaths and deaths[0] == seat else 0.0
    return {
...
def _evaluate_chunk(args):
    ...
    for _ in range(n):
        ...
        if deaths[0] == seat:
            first_deaths += 1
    return first_deaths, games


def evaluate(policy_w, n=2000, seed=99, ex=None):
    if ex is None:
        first_deaths, games = _evaluate_chunk((policy_w, n, seed))
    else:
        import math
        ...
        first_deaths = games = 0
        for f in futs:
            fd, g = f.result()
            first_deaths += fd; games += g
    rate = first_deaths / games if games else 0.0
    return rate
