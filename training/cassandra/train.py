...
from training.cassandra.net import (CassandraNet, GRU_H, ALL_KEYS, N_TARGETS,
                                     N_CARD_TYPES, N_BUCKETS, NOPE_CTX_DIM)
from training.cassandra.rollout import rollout_worker, evaluate
...
DEPLOY_OUT = os.path.join(HERE, '..', '..', 'agents', 'cassandra_weights.json')
...
def _ppo_coeff(new_logp, old_logp, adv, clip):
    ...
    return -(adv * ratio * (1.0 if use1 else float(in_range)))


def _entropy_dlogits(P, mask, coef):
    log_P = np.log(P + 1e-12)
    mean  = float(np.sum(P * (log_P + 1.0) * mask))
    return (-coef) * P * (mean - (log_P + 1.0) * mask)
...
def _game_grads(net, gd, clip, vf_coef, ent_coef):
    ...
    for step in gd['steps']:
        ...
        snap = np.array(step['snapshot'], dtype=np.float32)
        a2, tcache  = net.trunk_forward(hs[ec], snap)
        logits, val = net.policy_forward(a2)
        ...
        oh      = np.zeros(N_ACTIONS); oh[step['action']] = 1.0
        dlogits = d_logp * (oh - P) * mask + _entropy_dlogits(P, mask, ent_coef / max(n, 1))
        dvalue  = vf_coef * (val - step['return']) / max(n, 1)
        ...
        if step.get('has_target') and step.get('target_mask') is not None:
            ...
            d_tgt = _ppo_coeff(new_tgt_logp, step['target_logp'], step['advantage'], clip) / max(n, 1)
            oh_tgt = np.zeros(N_TARGETS); oh_tgt[ta] = 1.0
            dtgt = d_tgt * (oh_tgt - tgt_P) * tgt_mask
    ...
    # ---- want_to_nope steps (context-aware) ----
    ...
    for step in nsteps:
        ...
        snap = np.array(step['snapshot'], dtype=np.float32)
        a2, tcache  = net.trunk_forward(hs[ec], snap)
        nope_ctx    = np.array(step['nope_ctx'], dtype=np.float32)
        logit, ncache = net.nope_forward(a2, nope_ctx)
        prob   = _sigmoid(logit)
        dec    = step['decision']
        ...
        dvalue  = vf_coef * ((net.policy_forward(a2)[1]) - step['return']) / max(nn, 1)
        da2_nope, nope_g = net.nope_backward(d_logit, a2, nope_ctx, ncache)
    ...
    for step in gsteps:
        ...
        dvalue   = vf_coef * (net.policy_forward(a2)[1] - step['return']) / max(ng, 1)
    ...
    # ---- place_kitten steps (parity-context-aware) ----
    ...
    for step in psteps:
        ...
        snap = np.array(step['snapshot'], dtype=np.float32)
        a2, tcache   = net.trunk_forward(hs[ec], snap)
        parity       = step.get('deck_parity', 0)
        logits, pext = net.place_forward(a2, parity)
        P  = _softmax(logits)
        bi = step['decision']
        ...
        dlogits  = d_logp * (oh - P)
        dvalue   = vf_coef * (net.policy_forward(a2)[1] - step['return']) / max(np_, 1)
        da2_place, place_g = net.place_backward(dlogits, pext)
        da2_val,   val_g   = net.policy_backward(np.zeros(N_ACTIONS), dvalue, a2)
    ...
    for _ in range(epochs):
        ...
        if ex is None:
            ...
            if sum_grads and n_total > 0:
                avg = {k: v / n_total for k, v in sum_grads.items()}
                net.step(avg, lr)
...
def main():
    ...
    if args.resume and os.path.exists(CKPT_OUT):
        ...
        print('starting fresh (Cassandra GRU-192)', flush=True)

    pool = []
    base_rate = evaluate(net.policy_weights(), n=2000)
    best = base_rate
    print(f'baseline greedy vs losing-fleet: loss-rate {base_rate*100:.1f}%', flush=True)
        ...
        if it % 20 == 0:
            ...
            if rate > best:
                ...
                with open(BESTS_LOG, 'a') as _f:
                    _f.write(_j.dumps({'iter': it, 'loss_rate': round(rate*100, 2),
                                       't': int(time.time())}) + '\n')
            else:
                no_improve += 1
            rollout_loss = sum(1 for _, r in games if r > 0)
            print(f'iter {it:4d}  rollout_loss {rollout_loss/max(len(games),1)*100:4.1f}%  '
                  f'|  greedy vs losing-fleet: loss-rate {rate*100:5.2f}%  '
                  f'(best {best*100:.2f}%)  {time.time()-t0:.1f}s/it{tag}',
                  flush=True)
            if no_improve >= args.patience:
                print(f'stalled after {no_improve} evals without improvement. stopping.', flush=True)
                break
