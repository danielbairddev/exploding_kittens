...
class _LearnerAgent(GabrielAgent):
    """Inherits GabrielAgent's inference logic but runs trainable forward pass."""
...
def play_one(policy_w, pool_w, rng, npr, self_prob=0.3):
    ...
    # Reward: +1 for any non-win; 0 if Orpheus is the last survivor (winner)
...
def evaluate(policy_w, n=2000, seed=99, ex=None):
    if ex is None:
        ...
        for f in futs:
            l, g = f.result()
            losses += l; games += g
    rate = losses / games if games else 0.0
    return rate
