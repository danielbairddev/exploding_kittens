...
from training.orpheus.rollout import rollout_worker, evaluate
...
DEPLOY_OUT = os.path.join(HERE, '..', '..', 'agents', 'orpheus_weights.json')
...
def main():
    ...
    ap.add_argument('--self_prob', type=float, default=0.3,
                    help='prob of using past Orpheus snapshot as opponent (higher than Gabriel '
                         'because all opponents are already playing a similar style)')
    ...
    elif os.path.exists(DEPLOY_OUT) and not args.resume:
        # Bootstrap from current weights (gabriel copy) so we don't start from scratch
        import json
        with open(DEPLOY_OUT) as f: net.load_weights(json.load(f))
        print('bootstrapped from orpheus_weights.json', flush=True)
        ...
        if it % 20 == 0:
            ...
            if rate > best:
                ...
                with open(BESTS_LOG, 'a') as _f:
                    _f.write(_json.dumps({'iter': it, 'loss_rate': round(rate*100,2),
                                          't': int(time.time())}) + '\n')
            else:
                no_improve += 1
