...
- `ZeusAgent` (Zeus) — **benched.** Win-maximising twin of Hades: the *exact* same Transformer architecture (subclasses `HadesAgent`, reuses `training/hades/net.py` + encoders), trained with the opposite reward (+1 sole survivor / −1 otherwise, no shaping) vs the competitive fleet. Trainer: `training/zeus/`. Weights in `agents/zeus_weights.json`.
